...
public class SendMsgUtil {
	...
	public static String sendSMSHL(String num, String con) {
		...
		String ret = "";
		try {
			postData = "username=" + MessageUtil.getMsg(MessageCode.SMS_USER)
					+ "&password=" + MessageUtil.getMsg(MessageCode.SMS_PASS)
					+ "&phone=" + num + "&message="
					+ java.net.URLEncoder.encode(con, "gbk") + "&epid="
					+ MessageUtil.getMsg(MessageCode.SMS_EPID)
					+ "&linkid=&subcode=";
			logger.info(">>>>>>>>>>>>>>>>>>>>>>>>发送目标号码：" + num);
			logger.info(">>>>>>>>>>>>>>>>>>>>>>>>发送短信内容：" + con);
			ret = sendSMSGet(postData, MessageUtil.getMsg(MessageCode.SMS_URL));
	...
	public static String sendSMSGroup(String num, String con,String subcode) {
		...
		String postData;
		String ret = "";
		String username = MessageUtil.getMsg(MessageCode.SMS_USER_GROUP);
		String password = MessageUtil.getMsg(MessageCode.SMS_PASS_GROUP);
		
		try {
			if(!"".equals(subcode)){
				username = subcode;
				password = MessageUtil.getMsg(MessageCode.SMS_PASS_SON);
			}
			postData = "username=" + username
					+ "&password=" + password
					+ "&phone=" + num + "&message="
					+ java.net.URLEncoder.encode(con, "gbk") + "&epid="
					+ MessageUtil.getMsg(MessageCode.SMS_EPID_GROUP)
					+ "&linkid=&subcode=";
			logger.info(">>>>>>>>>>>>>>>>>>>>>>>>发送目标号码：" + num);
			logger.info(">>>>>>>>>>>>>>>>>>>>>>>>发送短信内容：" + con);
			ret = sendSMSGet(postData, MessageUtil.getMsg(MessageCode.SMS_URL));
	...
	public static String sendSMSHLTellingMall(String phoneNum, String smsContent) {
		...
		String ret = "";
		try {
			postData = "username=" + MessageUtil.getMsg(MessageCode.SMS_USER_TELLING_MALL)
					+ "&password=" + MessageUtil.getMsg(MessageCode.SMS_PASS_TELLING_MALL)
					+ "&phone=" + phoneNum + "&message="
					+ java.net.URLEncoder.encode(smsContent, "gbk") + "&epid="
					+ MessageUtil.getMsg(MessageCode.SMS_EPID_TELLING_MALL)
					+ "&linkid=&subcode=";
