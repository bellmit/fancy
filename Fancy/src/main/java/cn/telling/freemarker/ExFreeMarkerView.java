...
public class ExFreeMarkerView extends FreeMarkerView {
			...
			throws Exception
	{
		...
		/*
		 * 在这里我们默认生成静态文件,当ModelAndView有指定STATIC_HTML = false时,就不会输出HTML文件 例如：ModelAndView modelAndView = new
		 * ModelAndView("htmlTest"); modelAndView.addObject("STATICHTML", false);
		 */
	...
	{
		// 站点根目录的绝对路径
		String basePath = request.getSession().getServletContext().getRealPath("/sys/html");
		// model.get("glb");
