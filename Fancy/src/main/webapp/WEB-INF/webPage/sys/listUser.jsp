...
<div style="margin-bottom:5px">  
 <shiro:hasPermission name="/user/addUser.html"> 
  <a href="javascript:void(0)" class="easyui-linkbutton"  iconCls="icon-add" plain="true" id="addbtn">新增</a>  
        ...
        &nbsp; &nbsp;
           <shiro:hasPermission name="/user/eidtUserPwd.html"> 
   ...
   <form:form action="" method="post" id="searchForm">
    ...
    <table id="tt" class="easyui-datagrid" style="width:auto;height:320px"
            data-options="fit:true,collapsible:true,rownumbers:true,url:'<%=basePath%>user/viewAllUserJson.html',
            fitColumns:false,singleSelect:false,sortOrder:'desc',sortName:'clintname',pagination:true,toolbar:'#tb'"
      ...
      function editPanel(url,title){
        ...
        //post提交批量删除信息并弹出处理结果
					...
					function delDatas(delList, listId, address) {
						...
						if (address == 'setAcceptUser.do'
								|| address == 'setAcceptDkUser.do'
								|| address == 'setAcceptCkUser') {
							mes = "确定要设置？";
						}

						if (address == 'CancelAcceptUser.do'
								|| address == 'CancelAcceptUserDk.do'
								|| address == 'CancelAcceptUserCk.do') {
							mes = "确定要取消该用户的短信通知？";
						}
					...
					$(function() {
						...
						$("#addbtn").click(function() {
							addPanel('user/addUser.html', '新增用户');
						});
						...
						$("#editbtn").click(
								function() {
									...
									if (rows == 0) {
										...
										$.messager.alert("系统提示", "一次只能修改一条数据!",
												"info");
										return;
										...
										editPanel('user/eidtUserPwd.html?id='
												+ parkparams, '修改用户密码_'
												+ parkparams);
						...
						$("#addrolebtn").click(
								function() {
									...
									if (rows == 0) {
										...
										$.messager.alert("系统提示", "一次只能分配一条数据!",
												"info");
										return;
										...
										editPanel('aps.do?id=' + parkparams,
												'分配用户角色_' + parkparams);
						...
						$("#smsCancelDkbtn").click(
								function() {
									var rows = $("#tt").datagrid(
											"getSelections"); // 获取所有选中的行
									var parks = [];
									if (rows == 0) {
										$.messager.alert("系统提示", "请选择要取消的用户!",
												"info");
										return;
						...
						$("#addcompanybtn").click(
								function() {
									...
									if (rows == 0) {
										...
										editPanel('distributionCompany.do?id='
												+ parkparams, '分配用户公司_'
												+ parkparams);
						...
						$("#addContactsbtn").click(
								function() {
									...
									if (rows == 0) {
										...
										editPanel(
												'saveUserContactsRelation.do?id='
														+ parkparams,
												'分配用户联系人_' + parkparams);
