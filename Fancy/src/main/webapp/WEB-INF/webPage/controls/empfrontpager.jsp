...
<ol class="pageNum clearfix">
	...
	<a class="preBtn" href="javascript:void(0);">上一页</a>
	<% }else{ %>
	<a class="nextBtn" href="<%=pager.getPageUrl(pager.getLastPage())%>">上一页</a>
	<% } %>	
	</li>
	<% 
	int counter = 0;
	for(int i=pager.getStartPage(); i<=pager.getEndPage(); i++){
		...
		{%>
			<li><a class="crtPage" href="<%=pager.getPageUrl(i)%>"><%=i%></a></li>
		<%}else{%>		
