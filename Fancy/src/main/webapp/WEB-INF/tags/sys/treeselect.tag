...
<%@ attribute name="functionIndex" type="java.lang.String" required="false" description=""%>
<%@ attribute name="functionName" type="java.lang.String" required="false" description=""%>
	...
	$("#${id}Button, #${id}Name").click(function(){
		...
		top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}", "选择${title}", 300, 420, {
			ajaxData:{selectIds: $("#${id}Id").val()},buttons:{"确定":"ok", ${allowClear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				...
				//自定义方法名和方法参数
				if(typeof ${(functionName == null || functionName == '')? 'nofunctionName':functionName} == 'function'){
					${(functionName == null || functionName == '')? 'nofunctionName':functionName}('${(functionIndex == null || functionIndex == '')?'':functionIndex}',v);
				} 
