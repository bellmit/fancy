...
public class OracleAutoInjection {
	 ...
	 * @version V1.0
	 */
	private static void toInjection(Object obj, ResultSet rs, OracleLobHandler oracleLobHandler,
			Map<String, String> objInfoMap, Map<String, String> rsInfoMap) throws SQLException, SecurityException,
			NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException
	{
		...
		while (it.hasNext())
		{
			Map.Entry<String, String> entryObject = (Map.Entry<String, String>) it.next();
			// 结果集字段名称
			...
			while (itDB.hasNext())
			{
				...
				// 如果匹配(对象属性名称和字段名称)
				if (objName.equalsIgnoreCase(rsName))
				{// 如果字段类型为CHAR或者VARCHAR2
					// 进行注入
	 ...
	 * @version V1.0
	 */
	private static void reflectInjection(ResultSet rs, Object obj, String objName, String objType, String rsName,
			String rsType, OracleLobHandler oracleLobHandler) throws SecurityException, NoSuchMethodException,
			IllegalArgumentException, IllegalAccessException, InvocationTargetException, SQLException
	{
				...
				|| "INT".equalsIgnoreCase(rsType))
		{
			...
			{
				setMethod = clas.getDeclaredMethod(sb.toString(), Integer.class);
	...
	public static void main(String[] args) throws ClassNotFoundException, SQLException
	{
		...
		Connection conn = DriverManager.getConnection("jdbc:oracle:thin:@172.100.1.15:1521:b2btest1", "citsonline",
				"cits3011");
		...
		for (int i = 1; i <= rmd.getColumnCount(); i++)
		{
			System.out.println(rmd.getColumnName(i) + " == " + rmd.getColumnTypeName(i) + " == " + rmd.getPrecision(i)
					+ " ==1 " + rmd.getScale(i));
		}
		int i = 0;
		if (!rs.next())
